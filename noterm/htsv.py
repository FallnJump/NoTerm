...
if __name__=="__main__":
	...
	def _waitforstatus0(n=-1):
		while(True):
			time.sleep(0.5)
			u=_printreqtest("status?")
			u=int(u.split("\n")[0])
			n-=1
			if u==0 or n==0:
				break
	def _getreqtest(path,L=None,dt=None):
		testhost="http://localhost:8080/"
		b=clireq(testhost+path,req,dt)
		if L is not None:
			s=b[:L]
			s="length= "+str(1+len(b)//1024)+" kbytes\n"+str(s)
		else:
			s=b.decode(senc)
		return s
	def _printreqtest(path,L=None,dt=None):
		s=_getreqtest(path,L,dt)
		print(s)
		return s

	q=BytesIO(b"1234567890")
	while(1):
		bin=q.read(4)
		if bin==b"":
			break
		print(bin)
	sv=RunServer(8080,False)
	def thrun():
		sv.serve_forever()

	a=th.Thread(target=thrun)
	a.start()
	senc=sys.getfilesystemencoding()
	campath="/storage/emulated/0/DCIM/camera/"
	pypath="/storage/emulated/0/document/pycodes/"
	mv1="ttt/tus/yyy"
	mv2="ttt/tut/yyy"
	#_printreqtest("ls?"+campath+"?0-5")
	#_printreqtest("lsl?"+pypath+"?0-2")
	_printreqtest("mnt/",L=256)
	#_printreqtest("swapbk?/home/tfjsm/bakup/test.sh.backup.0001")
	exit()
	postb=b"echo 556\nsleep 2\necho 887"
	_printreqtest("cmd?",dt=postb)
	_printreqtest("cmd?sleep%205;echo%20999")
	_printreqtest("stop?[0]Cmd")
	#_printreqtest("")
	#_printreqtest("media?"+pypath+"DSC_0643.NEF&c",256)
	tt=glob(campath+"*.mp4")
	_printreqtest("movcv?"+tt[0])
	_srcp="../ttt/tus/yyy"
	_desp="/storage/emulated/0/document/../document/pycodes/ttt/tut/yyy"
	for tttt in range(3):
		_printreqtest("move?"+_srcp+":"+_desp)
		_waitforstatus0()
	_printreqtest("move?"+_desp+":"+_srcp)
	_waitforstatus0()
	#_printreqtest("status?"+tt[0]+"&c")
	exit()
	b=clireq("http://localhost:8080/thumb?/storage/emulated/0/DCIM/camera/?b0?5-6",req)
	s=b.decode(senc)
	print("ended")
	#import matplotib.pyplot as plt
	print(s)
	for iii in range(1):
		#b=clireq("http://localhost:8080/thumb?/storage/emulated/0/DCIM/camera/?=IMG_20201108_125653215.jpg|9?$IMG_20201101_140720040_HDR.jpg",req)
		b=clireq("http://localhost:8080/dl?/storage/emulated/0/DCIM/camera/IMG_20201108_125653215.jpg?IMG_20201101_140720040_HDR.jpg",req)
		s=b[:256]
		#s=b.decode(senc)
	#import matplotib.pyplot as plt
	print(s)
	#im=cv2.imread(s[4].split("|")[-1])[:,:,::-1]
	#plt.imshow(im)
	#plt.show()
	#url="http://localhost:8080/file?"+s[4].split("|")[-1]
	"""
	...
	"""
	for i in range(1):
		q="http://localhost:8080/thumb?/storage/emulated/0/document/pycodes/pycoco/SimpleCocos/testdt?b0?-"
		b=clireq(q,req)
		s=b.decode(senc)
		print("res=",s)
